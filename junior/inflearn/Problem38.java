...
public class Problem38 {
    public static void main(String[] args) {
        ...
        RecuritPrincess recuritPrincess = new RecuritPrincess(princes, k);
        int result = recuritPrincess.mainLogic();
        System.out.println(result);
    }
}

class RecuritPrincess {

    private int k;
    private int[] princes;
    private boolean[] isOut;

    public RecuritPrincess(int[] princes, int k) {
        this.k = k;
        this.princes = princes;
        isOut = new boolean[princes.length];
    }

    int mainLogic() {
        int pointer = 0, cnt = 1, out = 0;

        while(out < princes.length-1) {
            int temp = pointer > princes.length-1 ? pointer % (princes.length) : pointer;
            if(isOut[temp]) {
                pointer++;
                continue;
            }
